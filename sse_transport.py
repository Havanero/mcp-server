...
class SSETransport:
    ...
    def _setup_routes(self):
        ...
        self.app.router.add_post(
            "/stream/tools/{tool_name}", self._handle_tool_stream_post
        )
    ...
    async def _handle_tool_stream_post(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            logging.info(
                f"📄 POST stream request for {tool_name} with JSON: {arguments}"
            )
    ...
    async def _handle_tool_stream(self, request: web.Request) -> web.StreamResponse:
        ...
        try:
            ...
            if hasattr(self.plugin_manager, "execute_tool_stream"):
                ...
                async for progress in self.plugin_manager.execute_tool_stream(
                    tool_name, arguments
                ):
                    ...
                    await self._send_sse_message(
                        response, context, "progress", progress
                    )
                ...
                await self._send_sse_message(
                    ...
                    context,
                    "progress",
                    {"message": f"Executing {tool_name}...", "progress": 0.3},
                )
                ...
                logging.info(
                    f"✅ Tool {tool_name} completed with result: {type(result)}"
                )
    ...
    async def _handle_llm_stream(self, request: web.Request) -> web.StreamResponse:
        ...
        try:
            ...
            tokens = (
                prompt.split()
                if prompt
                else ["Hello", "world", "from", "streaming", "MCP"]
            )
            ...
            for i, token in enumerate(tokens):
                ...
                await self._send_sse_message(
                    ...
                    context,
                    "token",
                    {"token": token, "position": i, "total": len(tokens)},
                )
            ...
            await self._send_sse_message(
                response, context, "error", {"error": str(e), "model": model}
            )
    ...
    async def _handle_mcp_stream(self, request: web.Request) -> web.StreamResponse:
        ...
        try:
            ...
            # Create MCP request
            mcp_request = MCPRequest(id=context.stream_id, method=method, params=params)

            await self._send_sse_message(
                response, context, "started", {"method": method, "params": params}
            )
            ...
            await self._send_sse_message(
                response, context, "completed", {"message": "MCP method completed"}
            )

        except Exception as e:
            await self._send_sse_message(
                response, context, "error", {"error": str(e), "method": method}
            )
    ...
    async def _handle_list_tools(self, request: web.Request) -> web.Response:
        ...
        try:
            if not self.plugin_manager:
                return web.json_response(
                    {"error": "Plugin manager not configured"}, status=500
                )
    ...
    async def _handle_call_tool(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            return web.json_response({"tool": tool_name, "error": str(e)}, status=500)
        ...
        return web.json_response(
            {
                ...
                "tools_loaded": (
                    len(self.plugin_manager.loaded_tools) if self.plugin_manager else 0
                ),
            }
        )
        ...
        return web.json_response(
            {
                ...
                "streaming": {
                    "active_streams": len(self.active_streams),
                    "total_messages": sum(
                        ctx.message_count for ctx in self.active_streams.values()
                    ),
                },
                "plugins": (
                    self.plugin_manager.get_stats() if self.plugin_manager else {}
                ),
            }
        )
    ...
    async def _handle_stream_stats(self, request: web.Request) -> web.Response:
        ...
        return web.json_response(
            {"active_streams": len(self.active_streams), "streams": streams_info}
        )
...
eventSource.addEventListener('error', (event) => {
    ...
    async def start_server(self):
        ...
        logging.info(
            f"📡 Streaming endpoints: /stream/tools/*, /stream/llm, /stream/mcp"
        )
